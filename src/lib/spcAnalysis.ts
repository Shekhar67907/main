...
export const analyzeData = (params: {
  ...
  const cp = (analysis.metrics.usl - analysis.metrics.lsl) / (6 * analysis.metrics.stdDevWithin);
  const cpu = (analysis.metrics.usl - analysis.metrics.xBar) / (3 * analysis.metrics.stdDevWithin);
  const cpl = (analysis.metrics.xBar - analysis.metrics.lsl) / (3 * analysis.metrics.stdDevWithin);
  const cpk = Math.min(cpu, cpl);

  const processShift = cpk < 0.75 * cp ? "Yes" : "No";
  const processSpread = cp < 1 ? "Yes" : "No";
  const specialCausePresent =
    analysis.metrics.pp >= cp
      ? "Special Cause Detection impossible"
      : analysis.metrics.pp < 0.75 * cp
      ? "Yes"
      : "No";
