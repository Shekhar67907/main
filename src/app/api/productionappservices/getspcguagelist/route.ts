...
export async function GET(request: Request) {
  try {
    ...
    const apiUrl = `${BASE_URL}/api/productionappservices/getspcguagelist`;
    ...
    console.error("Error fetching gauge list data:", error);
    ...
    return NextResponse.json(
      { error: "Failed to fetch gauge list data" },
      { status: 500 }
    );
  }
}