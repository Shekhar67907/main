...
export function calculateAnalysisData(
  ...
  const xBarUcl = grandMean + constants.A2 * avgRange;  // UCL for X-bar chart
  const xBarLcl = grandMean - constants.A2 * avgRange;  // LCL for X-bar chart

  // Control limits for R-bar chart
  const rangeUcl = constants.D4 * avgRange;  // UCL for R-bar chart
  const rangeLcl = constants.D3 * avgRange;  // LCL for R-bar chart
